...
extract($_GET);
extract($_POST);
$sth = $db->query ("SELECT * FROM `job_category`");
$count = $sth->rowCount();
if(isset($_POST) && $_POST['action'] == 'updateStatus') {
  $sth = $db->query ("UPDATE `job_category` SET `display_status`='$display_status' WHERE `guid`=$guid");
	...
	$sth = $db->exec ("INSERT INTO `job_category`(`name`) VALUES ('$names')");
	}
	if($sth > 0) {
	$post_msg = '<h4 style="color: green;">Categories Successfully Updated</h4>';	
	header('location:'.URL.'job_category.php?post_msg='.$post_msg);
	} else { 
...
if(isset($_POST) && $_POST['submit'] == 'SaveEdit') {
	...
	$sth = $db->exec ("UPDATE `job_category` SET `name`='$name' WHERE `guid`=$guid");
	if($sth > 0) {
  ...
  <div id="main-container"> 
        ...
        <div class="col-lg-12"> 
          ...
          <div class="breadcrumbs">
            <ul class="breadcrumb">
              ...
              <li class="active"><a href="<?php echo URL; ?>job_category.php">Job Category</a></li>
            </ul>
            <div class="b-right hidden-xs">
              <ul>
                <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                  <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                    <li><a href="<?php echo URL; ?>job_category.php?action=addNew">Add New Category</a></li>
                    <li><a href="<?php echo URL; ?>job_category.php">View Job Categories</a></li>
                  </ul>
                </li>
              </ul>
            </div>
          ...
          <div class="page-header title"> 
            <!-- PAGE TITLE ROW -->
            <h1>Admin <span class="sub-title"><a href="job_category.php">Classified Category</a></span></h1>
          </div>
          <!-- /#ek-layout-button -->
	...
	$guid = $_GET['guid'];  ?>
          ...
          <div class="portlet">
            ...
            <div id="f-6" class="panel-collapse collapse in">
              <div class="portlet-body">
                <?php $sth = $db->prepare ("SELECT * FROM `job_category` WHERE `guid` = '$guid'");
			      $sth-> execute();
				  $row = $sth-> fetch();
				?>
                <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
      ...
      <?php } else if($action == 'deleteData') {
		$guid = $_GET['guid'];
		$sth = $db->query ("DELETE FROM `job_category` WHERE `guid`='$guid'");
		header('location:'.URL.'job_category.php'); 
	} else { ?>
      <div class="row">
        <div class="col-lg-12">
                  ...
                  <table id="SampleDT" class="datatable table table-hover table-striped table-bordered tc-table ">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td class="col-medium center"><div class="btn-group btn-group-sm"> <a href="<?php echo URL; ?>job_category.php?action=editData&guid=<?php echo $row[0]; ?>" class="btn btn-inverse"><i class="fa fa-edit icon-only"></i></a> <a href="<?php echo URL; ?>job_category.php?action=deleteData&guid=<?php echo $row[0]; ?>" class="btn btn-danger" onClick="return delete1();"><i class="fa fa-trash icon-only"></i></a></div></td>
                      </tr>
                      <?php $m++; } } ?>
                    </tbody>
                  </table>
