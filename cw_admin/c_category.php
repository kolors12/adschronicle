...
extract($_GET);
extract($_POST);
$sth = $db->query ("SELECT * FROM `c_category`");
$count = $sth->rowCount();
if(isset($_POST) && $_POST['action'] == 'updateStatus') {
  $sth = $db->query ("UPDATE `c_category` SET `display_status`='$display_status' WHERE `guid`=$guid");
	...
	$sth = $db->exec ("INSERT INTO `c_category`(`name`) VALUES ('$names')");
	}
	if($sth > 0) {
	$post_msg = '<h4 style="color: green;">Categories Successfully Updated</h4>';	
	header('location:'.URL.'c_category.php?post_msg='.$post_msg);
	} else { 
...
if(isset($_POST) && $_POST['submit'] == 'SaveEdit') {
	...
	$name = stripslashes($_POST['Name']);
        $fdelivery = stripslashes($_POST['fdelivery']);
        if(!empty($fdelivery)) { $fd = "Allow"; } else { $fd = "Not Allow"; }
	$sth = $db->exec ("UPDATE `c_category` SET `name`='$name' WHERE `guid`=$guid");
	if($sth > 0) {
        ...
        <div class="col-lg-12"> 
          ...
          <div class="breadcrumbs">
            <ul class="breadcrumb">
              ...
              <li class="active"><a href="<?php echo URL; ?>c_category.php">Classified Category</a></li>
            </ul>
              ...
              <ul>
                ...
                <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                  <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                    <?php if($count < 10) {?>
                    <li><a href="<?php echo URL; ?>c_category.php?action=addNew">Add New Category</a></li>
                    <?php } ?>
                    <li><a href="<?php echo URL; ?>c_category.php">View Classified Categories</a></li>
                  </ul>
                </li>
              </ul>
            </div>
          ...
          <div class="page-header title"> 
            <!-- PAGE TITLE ROW -->
            <h1>Admin <span class="sub-title"><a href="c_category.php">Classified Category</a></span></h1>
          </div>
          <!-- /#ek-layout-button -->
          ...
          <div class="portlet">
            ...
            <div id="f-6" class="panel-collapse collapse in">
              <div class="portlet-body">
                <?php $sth = $db->prepare ("SELECT * FROM `c_category` WHERE `guid` = '$guid'");
			      $sth-> execute();
				  $row = $sth-> fetch();
				?>
                <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                  ...
                  <table id="SampleDT" class="datatable table table-hover table-striped table-bordered tc-table ">
					...
					?>
                      <tr>
                        ...
                        <td class="col-medium center"><div class="btn-group btn-group-sm"> <a href="<?php echo URL; ?>c_category.php?action=editData&guid=<?php echo $row[0]; ?>" class="btn btn-inverse"><i class="fa fa-edit icon-only"></i></a> <a href="<?php echo URL; ?>c_category.php?action=deleteData&guid=<?php echo $row[0]; ?>" class="btn btn-danger" onClick="return delete1();"><i class="fa fa-trash icon-only"></i></a></div></td>
                      </tr>
                      <?php $m++; } } ?>
                    </tbody>
                  </table>
...
$.ajax({
		  type:"post",
		  url:"<?php //echo URL; ?>c_category.php",
		  data:"guid="+id+"&display_status="+status+"&action=updateStatus",
