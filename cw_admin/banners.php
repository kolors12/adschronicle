...
if(isset($_POST) && $_POST['submit']=='adddata') {
	...
	$sth = $db->query("INSERT INTO `banner`(`cid`, `position`, `web_url`, `image`, `c_title`) VALUES ('$cid', '$position', '$web_url', '$image', '$c_title')");
	if($sth == true) { ?>
	<script type="text/javascript">
			alert('Data Successfully Inserted');
			window.location="<?php echo URL; ?>banners.php";
			</script>
	<?php } else { ?>
	<script type="text/javascript">
...
if(isset($_POST) && $_POST['submit']=='editdata')
{
	...
	if(!empty($filename)) {
		...
		$img_path="../adminupload/".$image;
		move_uploaded_file($_FILES['images1']['tmp_name'], $img_path);
		$ith = $db->query("UPDATE `banner` SET `image`='$image' WHERE `guid`='$qid'");
	} 
    
	$sth = $db->query("UPDATE `banner` SET `cid`='$cid', `position`='$position', `web_url`='$web_url', `c_title`='$c_title' WHERE `guid`='$qid'");
	if($sth == true) { ?>
	<script type="text/javascript">
...
if(isset($_POST) && $_POST['action']=='delete') {
	//print_r($_POST); exit;
	 $d = $_POST['id'];
		    $del = $db->query("SELECT `image` FROM `banner` WHERE `guid`='".$d."'");
			$row = $del->fetch();
			$filepath = "../adminupload/".$row['image'];
			if(file_exists($filepath)) {
				unlink($filepath);
			}
			$sth = $db->query("DELETE FROM `banner` WHERE `guid`='".$d."'");
			if($sth == true) {
			  echo "1111";	 exit;
			} 
 }
      ...
      <div class="col-lg-12"> 
        ...
        <div class="breadcrumbs">
            <ul class="breadcrumb">
              ...
              <li class="active"><a href="<?php echo URL; ?>banners.php">Top Ads</a></li>
            </ul>
              ...
              <ul>
                ...
                <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                  <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                    <?php // if($count < 10) {?>
                    <li><a href="<?php echo URL; ?>banners.php?action=add">Add New Adds</a></li>
                    <?php //} ?>
                    <li><a href="<?php echo URL; ?>banners.php">View Adds</a></li>
                  </ul>
                </li>
              </ul>
            </div>
        ...
        <div class="page-header title"> 
          <!-- PAGE TITLE ROW -->
          <h1>Top Ads <!--<span class="sub-title">responsive photo gallery using colorbox</span>--></h1>
        </div>
...
<a href="<?php echo URL; ?>banners.php" class="btn btn-info">Back</a>
    <div class="row">
      <div class="col-lg-8"> 
        <!--portlet start--->
        <div class="portlet">
            ...
            <div class="portlet-body">
              <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
                <div class="form-group">
                  <label>Title</label>
                  <input type="text" class="form-control" name="c_title" required="" placeholder="Enter Add title"/>
                </div>
                <div class="form-group">
                  <label>City</label>
                ...
                <div class="form-group">
                  <label>Display Position</label>
                    <select class="form-control" name="position" required="">
                        ...
                        <?php 
                         $pos = array("1"=>"Top Left","2"=>"Top Middle","3"=>"Top Right","4"=>"Bottom Left","5"=>"Bottom Middle","6"=>"Bottom Right");
                         foreach($pos as $pos_key=>$pos_value){
                            echo '<option value="'.$pos_key.'">'.$pos_value.'</option>';
                         }
    ...
    <?php } 
	   ...
	   $sth = $db->query ('SELECT * FROM `banner` WHERE `guid`="'.$q.'"');
	   $row = $sth->fetch();
   ?>
<a href="<?php echo URL; ?>banners.php" class="btn btn-info">Back</a>
    <div class="row">
          ...
          <div id="f-12" class="panel-collapse collapse in" aria-expanded="true">
            <div class="portlet-body">
              <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
                <div class="form-group">
                  <label>Title</label>
                  <input type="text" class="form-control" name="c_title" value="<?php echo $row['c_title']?>" required="" placeholder="Enter Add title"/>
                </div>
                <div class="form-group">
                  <label>City</label>
                    <select class="form-control" name="cid" required="">
                        <option value="">Select City</option>
                        <?php $cth = $db->query ("SELECT * FROM `cities`");
                              while($crow = $cth->fetch()) {
                              echo "<option value='".$crow[0]."'"; if($row['cid']==$crow[0]){ echo " selected "; } echo ">".$crow['name']."</option>";
                              } ?>
                   </select>
                </div>
                <div class="form-group">
                  <label>Display Position</label>
                    <select class="form-control" name="position" required="">
                        ...
                        <?php 
                         $pos = array("1"=>"Top Left","2"=>"Top Middle","3"=>"Top Right","4"=>"Bottom Left","5"=>"Bottom Middle","6"=>"Bottom Right");
                         foreach($pos as $pos_key=>$pos_value){
                            echo '<option value="'.$pos_key.'"'; if( $row['position']==$pos_key ){ echo " selected "; } echo '>'.$pos_value.'</option>';
                         }
                ...
                <div class="form-group">
                  <label>Url</label>
                  <input type="text" class="form-control" name="web_url" value="<?php echo $row['web_url']?>"/>
                </div>
                <div class="form-actions">
    ...
    <?php } else { ?>
<a href="<?php echo URL; ?>banners.php?action=add" class="btn btn-info">Add New</a>
    <div class="row">
      <div class="col-lg-12">
        <div class="portlet">
          <div class="portlet-heading dark">
          ...
          <div class="padding-25 panel-collapse collapse in" id="f-1" aria-expanded="true"> 
            ...
            <ul class="tc-gallery-2 clearfix">
              <?php 
			 $sth = $db->query ('SELECT * FROM `banner`');
			 $count = $sth->rowCount();
			 if($count > 0) {
				 $m= 1;
				 while($row = $sth->fetch()) { 
              ...
              <li class="thumbnail thumbnail-box-<?php echo $row[0]; ?>">
                <div class="thumb-preview">
                  ...
                  <div class="gl-thumb-options"> <a class="gl-zoom" href="<?php if(empty($row['image'])) { echo URL.'assets/images/gallery/img1.jpg'; } else { echo URL.'../adminupload/'.$row['image']; } ?>" data-rel="colorbox" title="<?php echo $row[1]; ?>"> <i class="fa fa-search"></i> </a>
                    <div class="gl-toolbar"> 
                      <div class="gl-group pull-right"> <a href="<?php echo URL; ?>banners.php?action=edit&q=<?php echo $row[0]; ?>">Edit</a>
                ...
                <div class="gl-description"><small class="pull-left"><p><?php echo $row['c_title']; ?></p><a href="javascript:;" rel="nofollow"><?php echo $row['web_url']; ?></a></small></div>
              </li>
              <?php $m++; } } else {?>
...
<script type="text/javascript">	
	...
	function deleteme(s){
		...
		$.ajax({
				  ...
				  dataType:"text",
				  data:query,
				  url:"<?php echo URL; ?>banners.php",// url of php page where you are writing the query
