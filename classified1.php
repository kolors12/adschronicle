...
<?php include("header.php") ?>
    ...
    $ath = $db->query("SELECT * FROM `c_category` ORDER BY `guid` DESC Limit $start, $limit"); 
    $tth = $db->query("SELECT * FROM `c_category`"); 
    $total = $tth->rowCount();
    ?>
    ...
    <div class="col-md-8">
     <?php if($total > 0){
        ...
        while($c_row = $ath->fetch()){
            $add = $db->query("SELECT * FROM `adds_requests` WHERE `txtC_Category` LIKE '%".$c_row['name']."%' AND `txtC_City` = '".$_COOKIE['area']."' AND `status`='Active' ORDER BY `guid` DESC");
        ?>
