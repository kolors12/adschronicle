...
<?php include("header.php") ?>
    ...
    $ath = $db->query("SELECT * FROM `cat` ORDER BY `name` ASC Limit $start, $limit"); 
  ...
  <div class="row" style="padding:20px 0px 0px 0px;">
    ...
    <div class="col-md-10 col-md-offset-1">
        ...
        while($c_row = $ath->fetch()){
            ...
            $add = $db->query("SELECT * FROM `adds_requests` WHERE `txtC_Category` = '".$c_row['guid']."' AND `txtC_City` = '".$_COOKIE['area']."' AND `status`='Active' ");
        ?>
 ...
 //echo 	"SELECT * FROM `subv` WHERE `guid`='".$a_row['txtAdTitle']."'"; exit;
	 $a2 = $db->query("SELECT * FROM `subv` WHERE `guid`='".$a_row['txtAdTitle']."' order by `name` ASC ");
	 $b2=$a2->fetch();	?>
      <div class="shadow1" style="background-color:#eee; padding:10px; margin-bottom:25px">
        <p><strong><?php echo $b2['name'];?></strong></p>
...
<script>
  (adsbygoogle = window.adsbygoogle || []).push({
    google_ad_client: "ca-pub-6162010399219742",
