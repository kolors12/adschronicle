...
if(isset($_POST) && $_POST['submit']=='adddata') {
	...
	$sth = $db->query("INSERT INTO `bottom_adds`(`cid`, `category`, `adv_id`, `image`) VALUES ('$cid', '$category', '$adv_id', '$image')");
	if($sth == true) { ?>
	<script type="text/javascript">
			alert('Data Successfully Inserted');
			window.location="<?php echo URL; ?>bottom_adds.php";
			</script>
	<?php } else { ?>
	<script type="text/javascript">
...
if(isset($_POST) && $_POST['submit']=='editdata')
{
	...
	if(!empty($filename)) {
		...
		$img_path="../adminupload/".$image;
		move_uploaded_file($_FILES['images1']['tmp_name'], $img_path);
		$ith = $db->query("UPDATE `bottom_adds` SET `image`='$image' WHERE `guid`='$qid'");
	} 
    
	$sth = $db->query("UPDATE `bottom_adds` SET `cid`='$cid', `category`='$category', `adv_id`='$adv_id' WHERE `guid`='$qid'");
	if($sth == true) { ?>
	<script type="text/javascript">
...
if(isset($_POST) && $_POST['action']=='delete') {
	//print_r($_POST); exit;
	 $d = $_POST['id'];
		    $del = $db->query("SELECT `image` FROM `bottom_adds` WHERE `guid`='".$d."'");
			$row = $del->fetch();
			$filepath = "../adminupload/".$row['image'];
			if(file_exists($filepath)) {
				unlink($filepath);
			}
			$sth = $db->query("DELETE FROM `bottom_adds` WHERE `guid`='".$d."'");
			if($sth == true) {
			  echo "1111";	 exit;
			} 
 }
      ...
      <div class="col-lg-12"> 
        ...
        <div class="breadcrumbs">
            <ul class="breadcrumb">
              ...
              <li class="active"><a href="<?php echo URL; ?>bottom_adds.php">Bottom Ads</a></li>
            </ul>
              ...
              <ul>
                ...
                <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                  <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                    <?php // if($count < 10) {?>
                    <li><a href="<?php echo URL; ?>bottom_adds.php?action=add">Add New Adds</a></li>
                    <?php //} ?>
                    <li><a href="<?php echo URL; ?>bottom_adds.php">View Bottom Ads</a></li>
                  </ul>
                </li>
              </ul>
            </div>
        ...
        <div class="page-header title"> 
          <!-- PAGE TITLE ROW -->
          <h1>Bottom Ads <!--<span class="sub-title">responsive photo gallery using colorbox</span>--></h1>
        </div>
...
<a href="<?php echo URL; ?>bottom_adds.php" class="btn btn-info">Back</a>
    <div class="row">
      <div class="col-lg-8"> 
        <!--portlet start--->
        <div class="portlet">
    ...
    <?php } 
	   ...
	   $sth = $db->query ('SELECT * FROM `bottom_adds` WHERE `guid`="'.$q.'"');
	   $row = $sth->fetch();
   ?>
<a href="<?php echo URL; ?>bottom_adds.php" class="btn btn-info">Back</a>
    <div class="row">
              ...
              <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
                ...
                <div class="form-group">
                  <label>Category</label>
                    <select class="form-control" name="category" id="category" required="">
                        <option value="">Select Category</option>
                        <?php $cath = $db->query ("SELECT * FROM `super_category`");
                ...
                <div class="form-group">
                  <label>Advertiser/Title</label>
                    <select class="form-control" name="adv_id" id="adv_id" required="">
                        <option value="">Select Advertiser/Title</option>
                    <?php
                    	$ath = $db->query ("SELECT * FROM `add_services` WHERE `cid`='".$row['cid']."' AND `category`='".$row['category']."'"); 
                    ?>
                    	<?php while($arow = $ath->fetch()) {  
                          echo "<option value='".$arow['pid']."'"; if($row['adv_id']==$arow['pid']){ echo " selected "; } echo ">".$arow['add_title']."</option>";
                    	} 
    ...
    <?php } else { ?>
<a href="<?php echo URL; ?>bottom_adds.php?action=add" class="btn btn-info">Add New</a>
    <div class="row">
      <div class="col-lg-12">
        <div class="portlet">
          <div class="portlet-heading dark">
          ...
          <div class="padding-25 panel-collapse collapse in" id="f-1" aria-expanded="true"> 
            ...
            <ul class="tc-gallery-2 clearfix">
              <?php 
			 $sth = $db->query ('SELECT * FROM `bottom_adds`');
			 $count = $sth->rowCount();
			 if($count > 0) {
				 $m= 1;
				 while($row = $sth->fetch()) { 
				    $ath = $db->query("SELECT `add_title` FROM `add_services` WHERE `pid`='".$row['adv_id']."'");
                    $arow = $ath->fetch();
              ...
              <li class="thumbnail thumbnail-box-<?php echo $row[0]; ?>">
                <div class="thumb-preview">
                  ...
                  <div class="gl-thumb-options"> <a class="gl-zoom" href="<?php if(empty($row['image'])) { echo URL.'assets/images/gallery/img1.jpg'; } else { echo URL.'../adminupload/'.$row['image']; } ?>" data-rel="colorbox" title="<?php echo $row[1]; ?>"> <i class="fa fa-search"></i> </a>
                    <div class="gl-toolbar"> 
                      <div class="gl-group pull-right"> <a href="<?php echo URL; ?>bottom_adds.php?action=edit&q=<?php echo $row[0]; ?>">Edit</a>
                ...
                <div class="gl-description"><small class="pull-left"><a href="javascript:;" rel="nofollow"><?php echo $arow[0]; ?></a></small></div>
              </li>
              <?php $m++; } } else {?>
...
<script type="text/javascript">	
	...
	function deleteme(s){
		...
		$.ajax({
				  ...
				  dataType:"text",
				  data:query,
				  url:"<?php echo URL; ?>bottom_adds.php",// url of php page where you are writing the query
    ...
    $.ajax({
    		  ...
    		  data:"cid="+cid+"&id="+id+"&action=fetchexp8",
    			  success:function(response){
    				  $('#adv_id').html(response);
    				return true;
    		  }
    	   });
    });
