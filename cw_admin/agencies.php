...
extract($_GET);
extract($_POST);
$sth = $db->query ("SELECT * FROM `agencies`");
$count = $sth->rowCount();
if(isset($_POST) && $_POST['action'] == 'updateStatus1') {
  $sth = $db->query ("UPDATE `agencies` SET `featured`='$featured' WHERE `guid`=$guid");

} else if(isset($_POST) && $_POST['action'] == 'updateStatus2') {
  $sth = $db->query ("UPDATE `agencies` SET `hot`='$hot' WHERE `guid`=$guid");

}

else if(isset($_POST) && $_POST['Submit'] == 'Submit') {
	$date = date('Y-m-d');
	//print_r($_POST); exit;
	...
	// 	$image='';	
	// }
	$sth = $db->query ("INSERT INTO `agencies`(`name`,`address`) VALUES ('$name','$address')");
	if($sth > 0) {
	$post_msg = '<h4 style="color: green;">Agencies Successfully Updated</h4>';	
	header('location:'.URL.'agencies.php?post_msg='.$post_msg);
	} else { 
...
if(isset($_POST) && $_POST['submit'] == 'SaveEdit') {
	...
	$sth = $db->query ("UPDATE `agencies` SET `name`='$name',`address`='$address' WHERE `guid`=$guid");
	if($sth > 0) {
  ...
  <div id="main-container"> 
        ...
        <div class="col-lg-12"> 
          ...
          <div class="breadcrumbs">
            <ul class="breadcrumb">
              ...
              <li class="active"><a href="<?php echo URL; ?>agenciesphp">Adds Agencies</a></li>
            </ul>
              ...
              <ul>
                ...
                <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                  <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                    <?php if($count < 10) {?>
                    <li><a href="<?php echo URL; ?>agencies.php?action=addNew">Add New Agencies</a></li>
                    <?php } ?>
                    <li><a href="<?php echo URL; ?>agencies.php">View Agencies</a></li>
                  </ul>
                </li>
              </ul>
            </div>
          ...
          <div class="page-header title"> 
            <!-- PAGE TITLE ROW -->
            <h1>Admin <span class="sub-title"><a href="agencies.php">Adds Agencies</a></span></h1>
          </div>
          <!-- /#ek-layout-button -->
              ...
              <div class="portlet-body no-padding table-responsive">
                <div class="well white">
                  ...
                  <div class="form-group">
                    <label>Agencies Name</label>
                    <input type="text" class="form-control" name="name" required=""/>
                    
                  </div>
                  <div class="form-group">
                    <label>Agencies Address</label>
                   <textarea type="text" class="form-control" name="address" required="" row="2"> </textarea>
                  </div>
	...
	$guid = $_GET['guid'];  ?>
          ...
          <div class="portlet">
            ...
            <div id="f-6" class="panel-collapse collapse in">
              <div class="portlet-body">
                <?php $sth = $db->prepare ("SELECT * FROM `agencies` WHERE `guid` = '$guid'");
			      $sth-> execute();
				  $row = $sth-> fetch();
				?>
                <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
                  ...
                  <div class="form-group">
                    <label>Agencies Adress</label>
                    <textarea type="text" class="form-control" name="address" required=""><?php echo $row[2]; ?></textarea>
                  </div>
                  <div class="form-actions">
                    <input type="hidden" name="guid" value="<?php echo $guid; ?>" />
                
                    <button type="submit" class="btn btn-primary" value="SaveEdit" name="submit">Update</button>
      ...
      <?php } 
		...
		$sth = $db->query ("DELETE FROM `agencies` WHERE `guid`='$guid'");
		//$sth = $db->query ("DELETE FROM `sub_category` WHERE `sid`='$guid'");
	    //$sth = $db->query ("DELETE FROM `mini_category` WHERE `cid`='$guid'");
		//$sth->execute();
		header('location:'.URL.'agencies.php'); 
	} else { ?>
     <a href="<?php echo URL; ?>agencies.php?action=addNew" class="btn btn-info">Add New Agencies</a>
      <div class="row">
        <div class="col-lg-12">
                  ...
                  <table id="SampleDT" class="datatable table table-hover table-striped table-bordered tc-table ">
                    <thead>
                      <tr>
                        ...
                        <th data-class="expand" class="col-medium center">Agencies Name</th>
                        <th data-class="expand" class="col-medium center">Agencies Address</th>
					...
					?>
                      <tr>
                        ...
                        <td class="col-medium center"> <div class="form-group tcb"><label class="tcb-inline status-line" ><input type="radio" class="hot"  <?php if($row['hot']=='YES'){ echo "checked=''";  }?> value="YES" data-id="<?php echo $row[0]; ?>" /> Active </label>&nbsp;/&nbsp;<label class="tcb-inline status-line" ><input type="radio" class="hot" <?php if($row['hot']=='NO'){ echo "checked=''";  }?> value="NO" data-id="<?php echo $row[0]; ?>"/> Deactive </label></div></td>
                      
                        <td class="col-medium center"><div class="btn-group btn-group-sm"> <a href="<?php echo URL; ?>agencies.php?action=editData&guid=<?php echo $row[0]; ?>" class="btn btn-inverse"><i class="fa fa-edit icon-only"></i></a> <a href="<?php echo URL; ?>agencies.php?action=deleteData&guid=<?php echo $row[0]; ?>" class="btn btn-danger" onClick="return delete1();"><i class="fa fa-trash icon-only"></i></a></div></td>
                      </tr>
                      <?php $m++; } } ?>
                    </tbody>
                  </table>
