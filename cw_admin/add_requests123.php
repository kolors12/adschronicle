				 ...
				 <div class="form-group">
                  <label>Advertiser/Title</label>
                    <select class="form-control sid" name="txtAdTitle" id="sid" required="">
                       ...
                       <?php $cits = $db->query("SELECT * FROM `subv` ORDER BY `name` ASC");
                      while($cirs = $cits->fetch()){ ?>
                    <option value="<?php echo $cirs['sid'] ?>" <?php if($row['txtAdTitle']==$cirs['sid']){ echo "selected"; } ?>><?php echo $cirs['name'] ?></option>
                  <?php } ?>
