				 ...
				 <div class="form-group">
                  <label>Advertiser/Title</label>
                    <select class="form-control sid" name="txtAdTitle" id="sid" required="">
                       ...
                       <?php $citt = $db->query("SELECT * FROM `subv` ORDER BY `name` ASC");
                      while($ciroww = $citt->fetch()){ ?>
                    <option value="<?php echo $ciroww['sid'] ?>" <?php if($row['txtAdTitle']==$ciroww['sid']){ echo "selected"; } ?>><?php echo $ciroww['name'] ?></option>
                  <?php } ?>
                ...
                <table id="SampleDT" class="datatable table table-hover table-striped table-bordered tc-table ">
                  ...
                  <tbody>
                    <?php if($count > 0) { 
						...
						while($row = $sth->fetch()) {
                          ...
                          $lroo = $lthh->fetch();
						   ...
						   $lthhh = $db->query("SELECT * FROM `subv` WHERE `guid`='".$row['txtAdTitle']."'");
                          $lrooo = $lthhh->fetch();
					?>
                    ...
                    <tr>
                      ...
                      <b>Ad Category:</b> <?php echo $lroo['name']; ?><br /><b>Ad Title:</b> <?php echo $lrooo['name']; ?><br /><b>Ad Membership no:</b> <?php echo $row['txtMembership_no']; ?></small></td>
