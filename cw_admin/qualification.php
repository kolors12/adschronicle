...
if(isset($_POST) && $_POST['Submit'] == 'add') {
	...
	{
	$sth = $db->exec ("INSERT INTO `qualification`(`name`) VALUES ('$names')");
	}
	if($sth > 0) {
	$post_msg = '<h4 style="color: green;">Qualification Successfully Added</h4>';	
	header('location:'.URL.'qualification.php?post_msg='.$post_msg);
	} else { 
...
if(isset($_POST) && $_POST['submit'] == 'SaveEdit') {
	...
	$sth = $db->exec ("UPDATE `qualification` SET `name`='$name' WHERE `guid`=$guid");
...
<div id="wrapper">
  ...
  <div id="page-wrapper"> 
      ...
      <div class="col-lg-12"> 
        ...
        <div class="breadcrumbs">
          <ul class="breadcrumb">
            ...
            <li class="active"><a href="<?php echo URL; ?>qualification.php">Qualification</a></li>
          </ul>
            ...
            <ul>
              ...
              <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                  <li><a href="<?php echo URL; ?>qualification.php?action=addNew">Add New </a></li>
                  <li><a href="<?php echo URL; ?>qualification.php">View Qualification</a></li>
                </ul>
              </li>
            </ul>
          </div>
        ...
        <div class="page-header title"> 
          <!-- PAGE TITLE ROW -->
          <h1>Admin <span class="sub-title"><a href="qualification.php">qualification</a></span></h1>
        </div>
        ...
        <div class="portlet">
          ...
          <div id="f-6" class="panel-collapse collapse in">
            <div class="portlet-body">
              <?php $sth = $db->prepare ("SELECT * FROM `qualification` WHERE `guid` = '$guid'");
			      $sth-> execute();
				  $row = $sth-> fetch();
				?>
              <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
    ...
    <?php } 
		...
		$guid = $_GET['guid'];
		$ath = $db->query ("DELETE FROM `specialisation` WHERE `sid`='$guid'");
		$sth = $db->query ("DELETE FROM `qualification` WHERE `guid`='$guid'");
		//$sth->execute();
		header('location:'.URL.'qualification.php'); 
	} else { ?>
   <a href="<?php echo URL; ?>qualification.php?action=addNew" class="btn btn-info">Add New</a>
    <div class="row">
      ...
      <div class="col-lg-8">
        ...
        <?php  }
								$sth = $db->query ("SELECT * FROM `qualification`");
								$count = $sth->rowCount();
								// $count;
								 ?>
                ...
                <table id="SampleDT" class="datatable table table-hover table-striped table-bordered tc-table ">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="col-medium center"><div class="btn-group btn-group-sm"><a href="<?php echo URL; ?>specialisation.php?sid=<?php echo $row[0]; ?>" class="btn btn-inverse"><i class="fa fa-link icon-only"></i></a></div></td>
                      <td class="col-medium center"><div class="btn-group btn-group-sm"> <a href="<?php echo URL; ?>qualification.php?action=editData&guid=<?php echo $row[0]; ?>"  class="btn btn-inverse"><i class="fa fa-edit icon-only"></i></a> <a href="<?php echo URL; ?>qualification.php?action=deleteData&guid=<?php echo $row[0]; ?>" class="btn btn-danger" onClick="return delete1();"><i class="fa fa-trash icon-only"></i></a> </div></td>
                    </tr>
                    <?php $m++; } } ?>
                  </tbody>
                </table>
