...
if(isset($_POST) && $_POST['submit']=='editdata') {
      $filename = stripcslashes($_FILES['txtFile']['name']);
      	if(!empty($filename)) {
            ...
            unlink("../adminupload/".$imageLink);
 	        $sth = $db->query("UPDATE `image_classifieds` SET `txtFile`='$image' WHERE `guid`='$qid'");
     	}
    
	$sth = $db->query("UPDATE `image_classifieds` SET `txtAdTitle`='$txtAdTitle', `txtC_City`='$txtC_City', `txtUrl`='$txtUrl', `txtMembership_no`='$txtMembership_no' WHERE `guid`='$qid'");
	if($sth == true) { ?>
	<script type="text/javascript">
			alert('Data Successfully Updated');
			window.location="<?php echo URL; ?>image_classifieds.php";
			</script>
	<?php } else { ?>
	<script type="text/javascript">
    ...
    <div id="page-wrapper"> 
        ...
        <div class="col-lg-12"> 
          ...
          <div class="breadcrumbs">
            <ul class="breadcrumb">
              ...
              <li class="active"><a href="<?php echo URL; ?>image_classifieds.php">Image Classified Ads</a></li>
            </ul>
              ...
              <ul>
                ...
                <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                  <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                    <li><a href="<?php echo URL; ?>image_classifieds.php">View All</a></li>
                  </ul>
                </li>
              </ul>
            </div>
          ...
          <div class="page-header title"> 
            <!-- PAGE TITLE ROW -->
            <h1>Admin <span class="sub-title"><a href="<?php echo URL; ?>image_classifieds.php">Image Classified Ads</a></span></h1>
          </div>
          <!-- /#ek-layout-button -->
      ...
      <?php if(isset($_GET['action'])) {
		...
		$guid = $_GET['guid']; $status = $_GET['status']; 
                if($status=='Active'){
		$sth = $db->query ("UPDATE `image_classifieds` SET `status`='$status',`active_date`='".date('Y-m-d')."' WHERE `guid`='$guid'");
                } else { 
		$sth = $db->query ("UPDATE `image_classifieds` SET `status`='$status' WHERE `guid`='$guid'");

                }
		header('location:'.URL.'image_classifieds.php'); 
	} else if(isset($action) && $action=='editData') { 
       $q = $_GET['guid'];
	   $sth = $db->query ('SELECT * FROM `image_classifieds` WHERE `guid`="'.$q.'"');
	   $row = $sth->fetch();
   ?>
<a href="<?php echo URL; ?>image_classifieds.php" class="btn btn-info btn-xs">Back</a>
    <div class="row">
      <div class="col-lg-8"> 
        <!--portlet start--->
        <div class="portlet">
          <div class="portlet-heading dark">
            <div class="portlet-title">
              <h4>Edit Image Classified Ads</h4>
            </div>
          ...
          <div id="f-12" class="panel-collapse collapse in" aria-expanded="true">
            <div class="portlet-body">
               ...
               <div class="form-group">
                ...
                <select class="form-control required" name="txtC_City">
                  ...
                  <?php $citt = $db->query("SELECT * FROM `cities` ORDER BY `name` ASC");
                    ...
                    <option value="<?php echo $ciroww['guid'] ?>" <?php if($row['txtC_City']==$ciroww['guid']){ echo "selected"; } ?>><?php echo $ciroww['name'] ?></option>
                  <?php } ?>
                ...
                <div class="form-group">
                  <label>Advertiser/Title</label>
                    <input class="form-control" name="txtAdTitle"  value="<?php echo $row['txtAdTitle']?>" required="" />
                </div>
                <div class="form-group">
                  <label for="txtFile">File Upload:</label>
                  <input type="file" name="txtFile" class="form-control required" id="txtFile" /><br />
                  <img src="../adminupload/<?php echo $row['txtFile']; ?>" class="img-responsive" width="180px"/>
                </div>
                <div class="form-group">
                  <label>Web Url</label>
                    <input class="form-control" name="txtUrl"  value="<?php echo $row['txtUrl']?>" required="" />
                </div>
                <div class="form-group">
              ...
              <div class="form-actions">
                  ...
                  <input type="hidden" name="imageLink" value="<?php echo $row['txtFile']; ?>" />
    ...
    <?php } else if($action == 'deleteData') {
		$guid = $_GET['guid'];
		$cth = $db->query ("SELECT * FROM `image_classifieds` WHERE `guid`='$guid'");
        $crow = $cth->fetch();
        unlink("../adminupload/".$crow['txtFile']);
		$sth = $db->query ("DELETE FROM `image_classifieds` WHERE `guid`='$guid'");
		//$sth->execute();
		header('location:'.URL.'image_classifieds.php'); 
	} else { ?>
    <div class="row">
      <div class="col-lg-12">
        ...
        <?php  }
            $sth = $db->query ("SELECT * FROM `image_classifieds` ORDER BY `guid` DESC");
			$count = $sth->rowCount();
								// $count;
								 ?>
                ...
                <table id="SampleDT" class="datatable table table-hover table-striped table-bordered tc-table ">
                  <thead>
                    <tr>
                      ...
                      <th data-class="expand" class="center">Ad Image</th>
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="center">
                        <div class="form-group">
                        <label><input type="radio" onclick="window.location='<?php echo URL; ?>image_classifieds.php?action=changeStatus&guid=<?php echo $row[0]; ?>&status=Active'" <?php if($row['status']=='Active'){ echo "checked"; }?> /> Activate</label><br />
                        <label><input type="radio" onclick="window.location='<?php echo URL; ?>image_classifieds.php?action=changeStatus&guid=<?php echo $row[0]; ?>&status=DeActive'" <?php if($row['status']=='DeActive'){ echo "checked"; }?> /> DeActivate</label><br />
                        </div>
                        </td>
                      ...
                      <b>Ad Title:</b> <?php echo $row['txtAdTitle']; ?><br /><b>Web URL:</b> <?php echo $row['txtUrl']; ?><br /><b>Ad Membership no:</b> <?php echo $row['txtMembership_no']; ?></small></td>
                      <td class="center"><a href="../adminupload/<?php echo $row['txtFile']; ?>" target="_blank"><img src="../adminupload/<?php echo $row['txtFile']; ?>" class="img-responsive" width="180px"/></a></td>
                      ...
                      <td class="center"><div class="btn-group btn-group-xs"><a href="<?php echo URL; ?>image_classifieds.php?action=editData&guid=<?php echo $row[0]; ?>" class="btn btn-inverse"><i class="fa fa-edit icon-only"></i></a><a href="<?php echo URL; ?>image_classifieds.php?action=deleteData&guid=<?php echo $row[0]; ?>" class="btn btn-danger" onClick="return delete1();"><i class="fa fa-trash icon-only"></i></a> </div></td>
                    </tr>
                    <?php $m++; } } ?>
                  </tbody>
                </table>
