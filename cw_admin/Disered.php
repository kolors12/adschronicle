...
if(isset($_POST) && $_POST['Submit'] == 'add') {
	...
	{
	$sth = $db->exec ("INSERT INTO `desired_location`(`name`) VALUES ('$names')");
	}
	if($sth > 0) {
	$post_msg = '<h4 style="color: green;">City Successfully Added</h4>';	
	header('location:'.URL.'Disered.php?post_msg='.$post_msg);
	} else { 
...
if(isset($_POST) && $_POST['submit'] == 'SaveEdit') {
	...
	$sth = $db->exec ("UPDATE `desired_location` SET `name`='$name' WHERE `guid`=$guid");
...
<div id="wrapper">
  ...
  <div id="page-wrapper"> 
      ...
      <div class="col-lg-12"> 
        ...
        <div class="breadcrumbs">
          <ul class="breadcrumb">
            ...
            <li class="active"><a href="<?php echo URL; ?>Disered.php">Disered Location</a></li>
          </ul>
            ...
            <ul>
              ...
              <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                  <li><a href="<?php echo URL; ?>Disered.php?action=addNew">Add New </a></li>
                  <li><a href="<?php echo URL; ?>Disered.php">View Disered Location</a></li>
                </ul>
              </li>
            </ul>
          </div>
        ...
        <div class="page-header title"> 
          <!-- PAGE TITLE ROW -->
          <h1>Admin <span class="sub-title"><a href="Disered.php">Disered Location</a></span></h1>
        </div>
        ...
        <div class="portlet">
          ...
          <div id="f-6" class="panel-collapse collapse in">
            <div class="portlet-body">
              <?php $sth = $db->prepare ("SELECT * FROM `desired_location` WHERE `guid` = '$guid'");
			      $sth-> execute();
				  $row = $sth-> fetch();
				?>
              <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                <div class="form-group">
                  <label>Disered Location</label>
    ...
    <?php } 
		...
		$guid = $_GET['guid'];
		$ath = $db->query ("DELETE FROM `areas` WHERE `sid`='$guid'");
		$sth = $db->query ("DELETE FROM `desired_location` WHERE `guid`='$guid'");
		//$sth->execute();
		header('location:'.URL.'Disered.php'); 
	} else { ?>
    <div class="row">
      <div class="col-lg-8">
        ...
        <?php  }
								$sth = $db->query ("SELECT * FROM `desired_location`");
								$count = $sth->rowCount();
								// $count;
								 ?>
                ...
                <table id="SampleDT" class="datatable table table-hover table-striped table-bordered tc-table ">
                  <thead>
                    <tr>
                      ...
                      <th data-class="expand" class="col-medium center">Disered Location</th>
                      <!--<th data-class="expand" class="col-medium center">Areas</th>-->
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <!--<td class="col-medium center"><div class="btn-group btn-group-sm"><a href="<?php echo URL; ?>areas.php?sid=<?php echo $row[0]; ?>" class="btn btn-inverse"><i class="fa fa-link icon-only"></i></a></div></td>-->
                      <td class="col-medium center"><div class="btn-group btn-group-sm"> <a href="<?php echo URL; ?>Disered.php?action=editData&guid=<?php echo $row[0]; ?>"  class="btn btn-inverse"><i class="fa fa-edit icon-only"></i></a> <a href="<?php echo URL; ?>Disered.php?action=deleteData&guid=<?php echo $row[0]; ?>" class="btn btn-danger" onClick="return delete1();"><i class="fa fa-trash icon-only"></i></a> </div></td>
                    </tr>
                    <?php $m++; } } ?>
                  </tbody>
                </table>
...
<script type="text/javascript">
    ...
    document.getElementById('rowfield').value=dd; 
		...
		$("#customFields").append('<tr><td>Disered Location</td><td align="right"><div align="right">:</div></td><td><div align="center"><input type="text" name="name[]" required  class="form-control"></div></td><td><input value="0" name="chkRemoveAuthor[0]" type="checkbox"   class="form-control"/></td></tr>');
