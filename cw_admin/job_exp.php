...
extract($_GET);
extract($_POST);
$sth = $db->query ("SELECT * FROM `job_exp`");
$count = $sth->rowCount();
if(isset($_POST) && $_POST['action'] == 'updateStatus') {
  $sth = $db->query ("UPDATE `job_exp` SET `display_status`='$display_status' WHERE `guid`=$guid");
	...
	$sth = $db->exec ("INSERT INTO `job_exp`(`name`) VALUES ('$names')");
	}
	if($sth > 0) {
	$post_msg = '<h4 style="color: green;">Categories Successfully Updated</h4>';	
	header('location:'.URL.'job_exp.php?post_msg='.$post_msg);
	} else { 
...
if(isset($_POST) && $_POST['submit'] == 'SaveEdit') {
	...
	$sth = $db->exec ("UPDATE `job_exp` SET `name`='$name' WHERE `guid`=$guid");
	if($sth > 0) {
  ...
  <div id="main-container"> 
        ...
        <div class="col-lg-12"> 
          ...
          <div class="breadcrumbs">
            <ul class="breadcrumb">
              ...
              <li class="active"><a href="<?php echo URL; ?>job_exp.php">Total Experiences</a></li>
            </ul>
            <div class="b-right hidden-xs">
              <ul>
                <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                  <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                    <li><a href="<?php echo URL; ?>job_exp.php?action=addNew">Add New Category</a></li>
                    <li><a href="<?php echo URL; ?>job_exp.php">View Total Experiences</a></li>
                  </ul>
                </li>
              </ul>
            </div>
          ...
          <div class="page-header title"> 
            <!-- PAGE TITLE ROW -->
            <h1>Admin <span class="sub-title"><a href="job_exp.php">Total Experiences</a></span></h1>
          </div>
          <!-- /#ek-layout-button -->
	...
	$guid = $_GET['guid'];  ?>
          ...
          <div class="portlet">
            ...
            <div id="f-6" class="panel-collapse collapse in">
              <div class="portlet-body">
                <?php $sth = $db->prepare ("SELECT * FROM `job_exp` WHERE `guid` = '$guid'");
			      $sth-> execute();
				  $row = $sth-> fetch();
				?>
                <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
      ...
      <?php } else if($action == 'deleteData') {
		$guid = $_GET['guid'];
		$sth = $db->query ("DELETE FROM `job_exp` WHERE `guid`='$guid'");
		header('location:'.URL.'job_exp.php'); 
	} else { ?>
      <div class="row">
        <div class="col-lg-12">
                  ...
                  <table id="SampleDT" class="datatable table table-hover table-striped table-bordered tc-table ">
                    <thead>
                      <tr>
                        ...
                        <th data-class="expand" class="col-medium center">Exp</th>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td class="col-medium center"><div class="btn-group btn-group-sm"> <a href="<?php echo URL; ?>job_exp.php?action=editData&guid=<?php echo $row[0]; ?>" class="btn btn-inverse"><i class="fa fa-edit icon-only"></i></a> <a href="<?php echo URL; ?>job_exp.php?action=deleteData&guid=<?php echo $row[0]; ?>" class="btn btn-danger" onClick="return delete1();"><i class="fa fa-trash icon-only"></i></a></div></td>
                      </tr>
                      <?php $m++; } } ?>
                    </tbody>
                  </table>
...
<script type="text/javascript">
    ...
    document.getElementById('rowfield').value=dd; 
		...
		$("#customFields").append('<tr><td>Exp</td><td align="right"><div align="right">:</div></td><td><div align="center"><input type="text" name="name[]" required  class="form-control"></div></td><td><input value="0" name="chkRemoveAuthor[0]" type="checkbox"   class="form-control"/></td></tr>');
