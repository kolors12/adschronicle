...
if(isset($_POST) && $_POST['Submit'] == 'add') {
	...
	{
	$sth = $db->exec ("INSERT INTO `child_category`(`name`,`sid`, `mid`) VALUES ('$names','$sid','$mid')");
	}
	if($sth > 0) {
	$post_msg = '<h4 style="color: green;">Main Category added successfully </h4>';	
	header('location:'.URL.'child_category.php?sid='.$sid.'&mid='.$mid.'&post_msg='.$post_msg);
	} else { 
...
if(isset($_POST) && $_POST['submit'] == 'SaveEdit') {
	...
	$sth = $db->exec ("UPDATE `child_category` SET `name`='$name' WHERE `guid`=$guid");
 ...
 $tth = $db->query("SELECT s.name,m.name FROM `super_category` s, `main_category` m WHERE s.guid = '$sid' AND m.guid = '$mid'");
 $trow = $tth-> fetch();
?>
<!DOCTYPE html>
<html lang="en">
...
<div id="wrapper">
      ...
      <div class="col-lg-12"> 
        ...
        <div class="breadcrumbs">
          <ul class="breadcrumb">
            ...
            <li> <a href="javascript:;"><?php echo $trow[0]; ?></a></li>
            <li> <a href="<?php echo URL; ?>main_category.php?sid=<?php echo $sid; ?>"><?php echo $trow[1]; ?></a></li>
            <li class="active"><a href="<?php echo URL; ?>child_category.php?sid=<?php echo $sid; ?>&mid=<?php echo $mid; ?>">Child Category</a></li>
          </ul>
            ...
            <ul>
              ...
              <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                  <li><a href="<?php echo URL; ?>child_category.php?action=addNew&sid=<?php echo $sid; ?>&mid=<?php echo $mid; ?>">Add New</a></li>
                  <li><a href="<?php echo URL; ?>child_category.php?sid=<?php echo $sid; ?>&mid=<?php echo $mid; ?>">View Main Category</a></li>
                </ul>
              </li>
            </ul>
          </div>
        ...
        <div class="page-header title"> 
          <!-- PAGE TITLE ROW -->
          <h1><?php echo $trow[0]; ?>&nbsp;<a href="javascript:;"><?php echo $trow[1]?></a>&nbsp;<a href="<?php echo URL; ?>child_category.php?sid=<?php echo $sid; ?>&mid=<?php echo $mid; ?>">Child Category</a></h1>
        </div>
  ...
  ?>
        ...
        <div class="portlet">
          ...
          <div id="f-6" class="panel-collapse collapse in">
            <div class="portlet-body">
              <?php $sth = $db->prepare ("SELECT * FROM `child_category` WHERE `guid` = '$guid'");
			      $sth-> execute();
				  $row = $sth-> fetch();
				?>
              <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
    ...
    <?php } 
		...
		$guid = $_GET['guid']; $sid =$_GET['sid'];$mid = $_GET['mid'];
	
		$sth = $db->query ("DELETE FROM `child_category` WHERE `guid`='$guid'");
		//$sth->execute();
		header('location:'.URL.'child_category.php?sid='.$sid.'&mid='.$mid); 
	} else { ?>
    <div class="row">
      <div class="col-lg-8">
        ...
        <?php 
								...
								$sth = $db->query ("SELECT * FROM `child_category` WHERE sid='$sid' AND mid='$mid'");
								$count = $sth->rowCount();
								// $count;
								 ?>
        ...
        <div class="portlet table-responsive"><!-- /Portlet -->
          <div class="portlet-heading dark">
            <div class="portlet-title">
              <h4><?php echo $trow[0]; ?>-<?php echo $trow[1] ?>-Child Category</h4>
            </div>
                ...
                <table id="SampleDT" class="datatable table table-hover table-striped table-bordered tc-table ">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="col-medium center"><?php echo $row[3]; ?></td>
                      <td class="col-medium center"><div class="btn-group btn-group-sm"> <a href="<?php echo URL; ?>child_category.php?action=editData&guid=<?php echo $row[0]; ?>&sid=<?php echo $sid; ?>&mid=<?php echo $mid; ?>"  class="btn btn-inverse"><i class="fa fa-edit icon-only"></i></a> <a href="<?php echo URL; ?>child_category.php?action=deleteData&guid=<?php echo $row[0]; ?>&sid=<?php echo $sid; ?>&mid=<?php echo $mid; ?>" class="btn btn-danger"><i class="fa fa-trash icon-only"></i></a> </div></td>
                    </tr>
                    <?php $m++; } } ?>
                  </tbody>
                </table>
...
<script type="text/javascript">
    ...
    document.getElementById('rowfield').value=dd; 
		...
		$("#customFields").append('<tr><td>Category Name</td><td align="right"><div align="right">:</div></td><td><div align="center"><input type="text" name="name[]" required  class="form-control"></div></td><td><input value="0" name="chkRemoveAuthor[0]" type="checkbox"   class="form-control"/></td></tr>');
