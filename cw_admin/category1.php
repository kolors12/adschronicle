...
if(isset($_POST) && $_POST['Submit'] == 'add') {
	...
	{
	$sth = $db->exec ("INSERT INTO `cat`(`name`) VALUES ('$names')");
	}
	if($sth > 0) {
	$post_msg = '<h4 style="color: green;">City Successfully Added</h4>';	
	header('location:'.URL.'category1.php?post_msg='.$post_msg);
	} else { 
...
if(isset($_POST) && $_POST['submit'] == 'SaveEdit') {
	...
	$sth = $db->exec ("UPDATE `cat` SET `name`='$name' WHERE `guid`=$guid");
...
<div id="wrapper">
  ...
  <div id="page-wrapper"> 
      ...
      <div class="col-lg-12"> 
        ...
        <div class="breadcrumbs">
          <ul class="breadcrumb">
            ...
            <li class="active"><a href="<?php echo URL; ?>category1.php">Category Name</a></li>
          </ul>
            ...
            <ul>
              ...
              <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                  <li><a href="<?php echo URL; ?>category1.php?action=addNew">Add New </a></li>
                  <li><a href="<?php echo URL; ?>category1.php">View Category Name</a></li>
                </ul>
              </li>
            </ul>
          </div>
        ...
        <div class="page-header title"> 
          <!-- PAGE TITLE ROW -->
          <h1>Admin <span class="sub-title"><a href="category1.php">Category Name</a></span></h1>
        </div>
        ...
        <div class="portlet">
          ...
          <div id="f-6" class="panel-collapse collapse in">
            <div class="portlet-body">
              <?php $sth = $db->prepare ("SELECT * FROM `cat` WHERE `guid` = '$guid'");
			      $sth-> execute();
				  $row = $sth-> fetch();
				?>
              <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
    ...
    <?php } 
		...
		$guid = $_GET['guid'];
		$ath = $db->query ("DELETE FROM `subv` WHERE `sid`='$guid'");
		$sth = $db->query ("DELETE FROM `cat` WHERE `guid`='$guid'");
		//$sth->execute();
		header('location:'.URL.'category1.php'); 
	} else { ?>
    <a href="<?php echo URL; ?>category1.php?action=addNew" class="btn btn-info">Add New</a>
    <div class="row">
      <div class="col-lg-8">
        <?php if(!empty($post_msg)) { ?>
        <div class="page-header no-padding-bottom"> <?php echo $post_msg; ?> </div>
        <?php  }
								$sth = $db->query ("SELECT * FROM `cat`");
								$count = $sth->rowCount();
								// $count;
								 ?>
                ...
                <table id="SampleDT" class="datatable table table-hover table-striped table-bordered tc-table ">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="col-medium center"><div class="btn-group btn-group-sm"><a href="<?php echo URL; ?>sub.php?sid=<?php echo $row[0]; ?>" class="btn btn-inverse"><i class="fa fa-link icon-only"></i></a></div></td>
                      <td class="col-medium center"><div class="btn-group btn-group-sm"> <a href="<?php echo URL; ?>category1.php?action=editData&guid=<?php echo $row[0]; ?>"  class="btn btn-inverse"><i class="fa fa-edit icon-only"></i></a> <a href="<?php echo URL; ?>category1.php?action=deleteData&guid=<?php echo $row[0]; ?>" class="btn btn-danger" onClick="return delete1();"><i class="fa fa-trash icon-only"></i></a> </div></td>
                    </tr>
                    <?php $m++; } } ?>
                  </tbody>
                </table>
