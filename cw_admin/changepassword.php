...
if(isset($_POST) && $_POST['SaveEdit'] == 'Update') {
	//$date = date('Y-m-d');
		...
		$sth = $db->prepare("SELECT * FROM login WHERE `guid`='".$_SESSION['adc_logInUser']."'");
		$sth->execute();
    ...
    $oldpassword = base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, md5($row[securitykey]), $_POST['oldpassword'], MCRYPT_MODE_CBC, md5(md5($row[securitykey]))));
		$bth = $db->prepare("SELECT * FROM login WHERE `guid`='".$_SESSION['adc_logInUser']."' AND password = '$oldpassword'");
	...
	$uth = $db->exec ("UPDATE `login` SET `password`='$password',`securitykey`='$securitykey'  WHERE `guid`='".$_SESSION['adc_logInUser']."'");
